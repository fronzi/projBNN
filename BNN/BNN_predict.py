...
test_train_df.insert(1,'$C/m^{2} (eV)$',Y_post_mean)
test_train_df.insert(2,'Error',(Y_post_std)/2)
res=test_train_df.iloc[:, 0:3]
res.to_csv(str(path)+'2Mpredcount7.csv')
...
#    plt.xlim(-0,-0.8)
    ...
    plt.ylabel('$e_{33}$ (Train)', fontsize=24)
    plt.xlabel('Feature 1', fontsize=24)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.title("Test set")
    plt.savefig(str(path)+'PIEZO (TEST)'+str(count), bbox_inches='tight')
    plt.show()
...
#    plt.plot(y, y, "b-")
    ...
    plt.yticks(fontsize=16)
    plt.title("Test set")
    plt.ylabel('NN-TEST ($C/m^{2} (eV)$)', fontsize=24)
    plt.xlabel('DFT ($C/m^{2} (eV)$)', fontsize=24)
